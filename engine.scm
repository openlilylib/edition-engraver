...
(define-method (propunset->string (ps <propunset>))
  (format #f "~A\\unset ~A" (if (is-once ps) "\\once " "")
    (string-append (if (get-context ps) (format #f "~A." (get-context ps)) "") (format #f "~A" (get-symbol ps)))))
...
(define-method (oop->string (o <override>))
  (let* ((context-name (get-context o))
    ...
    (if (is-revert o)
        ...
        (string-append (if (is-once o) "\\once " "") "\\override " context-sname (format #f "~A " (get-grob o)) (format #f "#'~A" (get-prop o)) " = " (format #f "~A" (get-value o)))
        )))
(export oop->string)
; display override
